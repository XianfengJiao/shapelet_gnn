...
class Multi_ConvTransformer_clstoken(pl.LightningModule):
    def __init__(self, input_dim=17, output_dim=1, feature_embed_dim = 32, demo_dim = 4, demo_hidden_dim = 32, num_channels=[4,8,16,32], attention_hidden_dim=32, kernel_size=2, keep_prob=0.5, nhead=4, num_layers=3):
        super().__init__()
        # hyperparameters
        ...
        conv_transformers = [
            ConvTransformer(feature_embed_dim = self.feature_embed_dim, num_channels=self.num_channels, attention_hidden_dim=self.attention_hidden_dim, kernel_size=self.kernel_size, keep_prob=self.keep_prob, nhead=self.nhead, num_layers=self.num_layers) \
    ...
    def gen_key_padding_mask(self, shape, lens, demo_input):
        # Considering the cls_token
        shape = list(shape)
        shape[-1] += 1
        mask = torch.full(shape, False)
