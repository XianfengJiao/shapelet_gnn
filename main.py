...
def main(args):
    ...
    record_data, label_data = load_record(record_path=args.graph_path)
    print('#'*20,'End Loading Data','#'*20)
    
    kfold = StratifiedKFold(n_splits=args.kfold, shuffle=True, random_state=args.seed)
    kfold_metrics = {}
    for i, (train_set, val_set) in enumerate(kfold.split(record_data['f0'], label_data)):
        train_dataset = RecordDataset(
            input_data={key:np.array(value)[train_set] for key, value in record_data.items()}, 
            label_data=np.array(label_data)[train_set], 
            data_dir=os.path.dirname(args.graph_path), 
            type='train', 
            fold=i, 
            topk=5)
        ...
        valid_dataset = RecordDataset(
            ...
            label_data=np.array(label_data)[val_set], 
            data_dir=os.path.dirname(args.graph_path), 
            type='valid', 
            fold=i, 
            topk=5)
        ...
        graph_data = build_graph(graph_path=args.graph_path)
        
        # ----------------- Instantiate Model --------------------------
        graph_model = GNN(hidden_channels=64, out_channels=32)
        graph_model = to_hetero(graph_model, graph_data.metadata(), aggr='sum')
        motality_model = MC_GRU(feature_size=4, input_dim=32, hidden_dim=64, output_dim=1)
        # ----------------- Instantiate Trainer ------------------------
        ...
        copyfile(trainer.best_metric_graph_model_path, trainer.best_metric_graph_model_path.replace('.pth', '_'+str(trainer.best_metric)+'.pth'))
        copyfile(trainer.best_metric_motality_model_path, trainer.best_metric_motality_model_path.replace('.pth', '_'+str(trainer.best_metric)+'.pth'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--log_dir', default='/home/jxf/code/Shapelet_GNN/logs/debug', type=str)
    parser.add_argument('--ckpt_save_path', default='/home/jxf/code/Shapelet_GNN/checkpoints/debug', type=str)
    parser.add_argument('--lr', default=1e-3, type=float)
    parser.add_argument('--batch_size', default=128, type=int)
    parser.add_argument('--graph_path', default='/home/jxf/code/pytorch-lightning-med/med-seq/graph_utils/gen_graph_data/test_shapelet_gnn', type=str)
