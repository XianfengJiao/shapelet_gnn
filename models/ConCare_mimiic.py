...
class SingleAttention(nn.Module):
    ...
    def forward(self, input, demo=None):
        ...
        time_decays = torch.tensor(range(time_step-1,-1,-1), dtype=torch.float32).unsqueeze(-1).unsqueeze(0).to(input.device)# 1*t*1
...
class ConCare_mimiic(nn.Module):
    def __init__(self, input_dim, hidden_dim, d_model=64,  MHD_num_head=4, demo_dim=12, d_ff=256, output_dim=1, keep_prob=0.5):
        super(ConCare_mimiic, self).__init__()
        ...
        # layers
        self.PositionalEncoding = PositionalEncoding(self.hidden_dim, dropout = 0, max_len = 400)
    ...
    def forward(self, input, lens, demo_input):
        ...
        # forward
        GRU_embeded_input = self.GRUs[0](input[:,:,0].unsqueeze(-1), Variable(torch.zeros(batch_size, self.hidden_dim).unsqueeze(0)).to(input.device))[0] # b t h
        Attention_embeded_input = self.LastStepAttentions[0](GRU_embeded_input)[0].unsqueeze(1)# b 1 h
        for i in range(feature_dim-1):
            embeded_input = self.GRUs[i+1](input[:,:,i+1].unsqueeze(-1), Variable(torch.zeros(batch_size, self.hidden_dim).unsqueeze(0)).to(input.device))[0] # b 1 h
