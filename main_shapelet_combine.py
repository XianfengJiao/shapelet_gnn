...
from models import GNN, MC_GRU_Combine
from trainer import Shapelet_Combine_Trainer
from datasets import RecordDataset

def load_emb_kfold(data_path, kfold):
    train_emb = pkl.load(open(os.path.join(data_path, 'kfold-'+str(kfold), 'seed5_saved_emb', 'train_emb.pkl'), 'rb'))
    valid_emb = pkl.load(open(os.path.join(data_path, 'kfold-'+str(kfold), 'seed5_saved_emb', 'valid_emb.pkl'), 'rb'))
    valid_y = pkl.load(open(os.path.join(data_path, 'kfold-'+str(kfold), 'seed5_saved_emb', 'valid_y.pkl'), 'rb'))
    train_y = pkl.load(open(os.path.join(data_path, 'kfold-'+str(kfold), 'seed5_saved_emb', 'train_y.pkl'), 'rb'))
    # valid_pred = pkl.load(open(os.path.join(data_path, 'kfold-'+str(kfold), 'seed5_saved_emb', 'valid_pred.pkl'), 'rb'))
    # train_pred = pkl.load(open(os.path.join(data_path, 'kfold-'+str(kfold), 'seed5_saved_emb', 'train_pred.pkl'), 'rb'))
    
    return train_emb, train_y, valid_emb, valid_y
...
def main(args):
    ...
    for i, (train_set, val_set) in enumerate(kfold.split(x, y)):
        ...
        train_emb, train_y, valid_emb, valid_y = load_emb_kfold(args.emb_data_path, i)
        
        assert(np.sum(train_y-np.array(y)[train_set]) == 0)
        ...
        motality_model = MC_GRU_Combine(feature_size=17, input_dim=32, hidden_dim=64, shape_dim=32, emb_dim=32, output_dim=1)
        # ----------------- Instantiate Trainer ------------------------
        trainer = Shapelet_Combine_Trainer(
...
if __name__ == '__main__':
    ...
    parser.add_argument('--emb_data_path', default='/home/jxf/code/Shapelet_GNN/logs/motality/ckd/20220928-12_ckd_Multi_ConvGRU_attn_keep1_lr5e-4_b64_k4_c16,32,64_auprc', type=str)
