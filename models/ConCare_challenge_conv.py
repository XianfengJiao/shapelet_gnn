...
from utils.model_utils import PositionalEncoding, TemporalBlock, Attention
...
class ConCare_challenge_conv(nn.Module):
    def __init__(self, input_dim=17, hidden_dim=32, num_channels=[4,8,16,32], kernel_size=4, d_model=32,  MHD_num_head=4, d_ff=64, output_dim=1, keep_prob=0.5):
        super(ConCare_challenge_conv, self).__init__()
...
#         demo_main = self.tanh(self.demo_proj_main(demo_input)).unsqueeze(1)# b hidden_dim
        ...
        contexts = self.SublayerConnection(contexts, lambda x: self.PositionwiseFeedForward(contexts)) # batch_size * d_input * hidden_dim
